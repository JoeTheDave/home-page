...
This project uses a **shared PostgreSQL container** approach to avoid port conflicts and abandoned containers when working with multiple home-page projects.

### How it works
- Single PostgreSQL container (`home-page-postgres`) shared across all projects
- Each project gets its own database (`home_page_dev`) within the shared container
...
To run multiple home-page projects simultaneously, adjust the ports in each project's `.env` file:
...
fly postgres create --name home-page-db
